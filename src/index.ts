...
type LoaderPhase = 'starting' | 'active' | 'stopping' | 'failed'
 ...
 */
export function resolveObuCommand(
  configured?: string,
  ...
  if (runtimePlatform !== 'win32') return 'obu'
...
export function identityForAgent(agentId: string): {
...
export function buildMcpArgs(config: Config, browserSessionId: string): string[] {
...
export async function loadObuSkill(skillPath = bundledSkillPath): Promise<SkillRegistration> {
...
function renderActivationInstructions(skill: SkillRegistration, state: LoaderState): string {
  const integration = [
...
function visibleToolCount(ctx: Context, agent: Agent, prefix: string): number {
  return ctx.tools.schemas(scopeOf(agent.ctx)).filter((schema) => schema.name.startsWith(prefix)).length
}

function selectorSummary(config: Config): string {
  const selectors = []
  if (config.browser) selectors.push(`browser=${config.browser}`)
  if (config.profile) selectors.push(`profile=${config.profile}`)
  if (config.socketDir) selectors.push(`socketDir=${config.socketDir}`)
...
export function createLoaderPlugin(dependencies: LoaderDependencies = {
  ...
  return {
    name,
    inject,
    Config: Config as Plugin.Object<Config>['Config'],
    apply(ctx, config) {
      ...
      ctx.commands.register({
        ...
        async handler(invocation: CommandInvocation) {
          ...
          if (action === 'status') {
            ...
            if (!state) return { kind: 'success', text: 'Open Browser Use is disabled for this session.' }
