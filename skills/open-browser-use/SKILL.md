...
## DSH On-Demand Integration

When this skill is injected by the DSH `/obu` command, the MCP server is already running for the current DSH session and its browser session id is already fixed. Use the dynamic tool prefix stated immediately before this skill body (for example `mcp__obu_<session>__`). Do not assume a globally fixed MCP namespace and do not launch another `obu mcp` process. Before opening a tab, call the prefixed `user_tabs`; finish browser work with the prefixed `finalize_tabs` exactly once.
